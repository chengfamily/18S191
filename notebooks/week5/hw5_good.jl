...
function matvec(M::RankOneMatrix, x)
	...
	return (M.w ⋅ x) * M.v # Your code here
end
...
begin
	...
	RankTwoMatrix(v1::AbstractVector{T} where T, v2::AbstractVector{T} where T) = RankTwoMatrix(RankOneMatrix(v1), RankOneMatrix(v2))
end
...
RankTwoMatrix(1.0:1.0:10.0, 0.0:0.1:0.9)

# ╔═╡ 9dfcae7a-8db7-11eb-3edb-f3819a94b81a
RankTwoMatrix(1.0:1.0:10.0, 0.0:0.1:0.9).B[1][1]
...
function Base.getindex(M::RankTwoMatrix{T} where T, i, j)
	return getindex(M.A, i, j) + getindex(M.B, i, j)
	#return M.A[i][j]+M.B[i][j] # Your code here
end

# ╔═╡ 0bab818e-8503-11eb-02b3-178098599847
function Base.size(M::RankTwoMatrix{T} where T)
